...
# (e.g. those in the NCBI nr database, having >gi|[0-9]|.* as their identifier),
# this script uses Entrez tools to find corresponding coding sequences where
# possible.
#
...
def parse_cmdline(args):
    ...
    parser.add_option("--sto", dest="stockholm",
                      action="store_true", default=False,
                      help="Parse .*/start-end identifiers as sequence regions")
    ...
    for s in SeqIO.parse(fhandle, 'fasta'):
        ...
        if re.match(regex, s.id) is None:
            ...
            resdict[s.id] = {'gid': s.id.split('|')[1],
                             'sid': s.id, 'aa': s.seq}
        if limit and len(resdict) >= limit: break    # For testing
...
def load_gb_cache(filename):
    ...
    for line in data:
        if len(line) != 3:
            ...
            logger.error(last_exception)
            sys.exit(1)
        cachedict[line[0]] = {'len': int(line[1]), 
                              'gi': line[2]}
    return cachedict
...
def cache_gb_headers(resdict, elink_cachename, acc_cachename, gb_cachename, 
    ...
    if not len(elink_notincache):  # Nothing left to do
        ...
        logger.info("%d unique ELink IDs not in cache" % len(elink_notincache))
...
def cache_gb_full(resdict, elink_cachename, acc_cachename, gb_cachename, 
    ...
    accset = \
      set([a for e, a in elink_accessions.items()]).intersection(gbfull_keyset)
    ...
    for qid, match_ids in \
        ...
        #    aux[acc] = gb_index[acc]
        #records = sorted([(len(aux[acc]), aux[acc]) for acc in \
        #                      match_accessions])
        records = sorted([(gb_index[acc]['len'], gb_index[acc]['gi']) \
                              for acc in match_accessions])
        # Find GI number of shortest record, and add to list
        #shortest_genbank_ids.add(records[0][1].annotations['gi'])
    ...
    if options.infilename is None:
        ...
        try:
            ...
            logger.error("Could not open input file: %s (exiting)" % \
                             options.infilename)
            logger.error(''.join(traceback.format_exception(sys.last_type, 
                                                         sys.last_value, 
                                                         sys.last_traceback)))
            sys.exit(1)
